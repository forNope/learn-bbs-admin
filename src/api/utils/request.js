...
export default function request(config) {
  ...
  http
    ...
    .then(({ status, data }) => {
      ...
      collector.emit(httpStatusCollection[+String(status)[0] - 1], data)
    ...
    .catch(err => {
      if (!axios.isCancel(err)) {
        if (err.response) {
          ...
          collector.emit(httpStatusMap[status], err)
          collector.emit(httpStatusCollection[+String(status)[0] - 1], err)
        }
